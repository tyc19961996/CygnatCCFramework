...
export class GlobalEvent {
    ...
    public static on(name: string, callback: (...args: any[]) => void, target?: any): number {
        return this.event.on(name, callback, target);
    }

    /**
     * 添加一个只触发一次的事件
     */
    public static once(name: string, callback: (...args: any[]) => void, target?: any): number {
        return this.event.once(name, callback, target);
    }
    /**
     * 发送一个事件
     * @param name 事件名称
    ...
    public static emit(name: string, ...args: any[]): void {
        this.event.emit(name, null, ...args);
    }
     ...
     * @param args 事件参数
     */
    public static emitToTarget(name: string, target: any, ...args: any[]) {
        this.event.emit(name, target, ...args);
    }
