...
export class HeaderManager {
     ...
     */
    public static async requestHeader(windowName: string, headerInfo: HeaderInfo<any> | null): Promise<void> {
        // 保存header的信息
        if (!headerInfo) {
            return;
        }
        ...
        if (!this._headers.has(headerName)) {
            const header = await this.createHeader(headerInfo);
    ...
    private static async createHeader(headerInfo: HeaderInfo<any>): Promise<IHeader> {
        // 创建header实例
        const info = InfoPool.getHeader(headerInfo.name);
        const headerPrefab = await AssetLoader.loadFormPrefab(info.bundle, headerInfo.name);
        const header = instantiate(headerPrefab);
        const headerBase = header.getComponent(Header);
    ...
    private static adjustHeaderPosition(headerName: string, windowName: string): void {
        const header = this._headers.get(headerName) as Header;
        ...
        // 调整层级：找到该组中最上层的显示窗口
        let maxWindowIndex = window.node.getSiblingIndex();
        for (let i = group.windowNames.length - 1; i >= 0; i--) {
            const win = WindowManager.getWindow<WindowBase>(group.windowNames[i]);
            if (win && win.isShowing()) {
                maxWindowIndex = Math.max(maxWindowIndex, win.node.getSiblingIndex());
            }
        }

        header.node.setSiblingIndex(maxWindowIndex + 1);
    }
}