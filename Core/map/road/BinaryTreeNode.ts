...
export default class BinaryTreeNode {
    ...
    public addTreeNode(roadNode:RoadNode,head:RoadNode = null)
    {
        ...
        var leftOrRight:number = 1; // leftOrRight值的状态：0 代表放左边，1代表放右边

        if(roadNode.f > head.f)
        {
            //f值大的放右边
            leftOrRight = 1;
        }else if(roadNode.f < head.f)
        {
            //f值小的放左边
            leftOrRight = 0;
        }else
        {
            //f值相等的情况
            if(roadNode.h < head.h)
            {
                //谁的h值最小谁放左边，即谁靠近目标点优先谁
                leftOrRight = 0; 
            }else if(roadNode.h > head.h)
            {
                //谁的h值最大谁放右边，即谁远离目标点优先级放后
                leftOrRight = 1; 
            }else
            {
                //f值和h值都相同的情况，谁靠近起始点优先谁，即看谁的g值最小
                if(roadNode.g < head.g)
                {
                    leftOrRight = 0; 
                }else
                {
        ...
        //根据leftOrRight值的状态，处理新增节点放在二叉堆结构父节点的左边还是右边。     备注-- leftOrRight值的状态： 0 代表放左边，1代表放右边
        if(leftOrRight == 1) 
        {
            if(head.right == null)
            {
