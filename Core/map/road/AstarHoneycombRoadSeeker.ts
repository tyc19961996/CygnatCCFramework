...
export default class AstarHoneycombRoadSeeker implements IRoadSeeker {
	...
	*/		
	 ...
	 * @return 
	 */		
	public seekPath2(startNode:RoadNode,targetNode:RoadNode,radius:number):Array<RoadNode>
	{
		this._startNode = startNode;
		this._currentNode = startNode;
		this._targetNode = targetNode;
		...
		while(true)
		{
			...
			}else
			{
				...
				{
					if(this._currentNode.g < closestNode.g)
					{
						closestNode = this._currentNode;
					}
				}
	...
	public isCanPass(node:RoadNode):boolean
	{
		...
		var honeyPoint:HoneyPoint = this.getHoneyPoint(node); //_neighbours是用六边形格子坐标hx,hy，不是用寻路的cx，cy坐标，所以要转换

		for(var i:number = 0 ; i < this._neighbours.length ; i++)
		{
			var hx:number = honeyPoint.hx + this._neighbours[i][0];
			var hy:number = honeyPoint.hy + this._neighbours[i][1];
			var node2:RoadNode = this.getNodeByHoneyPoint(hx,hy);
	...
	public getNeighbours(radius:number):number[][]
	{
		...
		}else
		{
			...
			var hasCheckDic:{[key:string]:boolean} = {}; 
			var hasSaveDic:{[key:string]:boolean} = {}; 
			var hround:number[][] = [[-1,-1],[-1,0],[0,1],[1,1],[1,0],[0,-1]]; //相邻点向量 
	 ...
	 * @returns 
	 */
	private getNeighboursRecursive(hx:number,hy:number,radius:number,checkRadius:number, outNeighbours:number[][],
		hasCheckDic:{[key:string]:boolean},hasSaveDic:{[key:string]:boolean}, hround:number[][])
	{
		...
		var key:string = hx + "_" + hy;
		hasCheckDic[key] = true;

		var len:number = hround.length;

		for(var i = 0; i < len ; i++)
		{
			var newHx = hx + hround[i][0];
			var newHy = hy + hround[i][1];
			...
			if(!hasSaveDic[key])
			{
				outNeighbours.push([newHx,newHy]);
				hasSaveDic[key] = true;
			}
